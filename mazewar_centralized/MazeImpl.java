...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        public synchronized void addRemoteClient(Client client){
        	assert(client != null);
        	Point point = new Point(client.getX(), client.getY());
        	CellImpl cell = getCellImpl(point);
        	addClient(client,point,client.getDirection());
        }
        ...
        private synchronized void addClient(Client client, Point point, int direction){
        	  assert(client != null);
              assert(checkBounds(point));
              CellImpl cell = getCellImpl(point);
              Direction d;
