...
public class Mazewar extends JFrame {
         ...
         */
        public Mazewar(String host, int port) throws IOException, ClassNotFoundException {
				...
				//list of clients
					...
					while((confirm = (InstructionPacket)in.readObject()).request_type != InstructionPacket.SERVER_DONE){
						...
						RemoteClient new_client = new RemoteClient(confirm.client_id, confirm.pos_x, confirm.pos_y, confirm.orientation);
				...
				//enqueueing thread
				new ServerListeningThread(in, actions, name).start();
				
                guiClient = new GUIClient(name, out);
                //consolePrintLn("putting "+this.getName()+" into the hash");
                ...
                confirm.location = new ClientLocation(host, port);
                ...
                new ClientDequeThread(actions,clients,maze).start();
